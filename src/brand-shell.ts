...
function applyBrand() {
  ...
  const homeLink = document.querySelector<HTMLAnchorElement>('.playground-topbar .brand');
  if (homeLink && homeLink.dataset.homeBound !== 'true') {
    homeLink.dataset.homeBound = 'true';
    homeLink.href = './';
    homeLink.setAttribute('aria-label', 'Build My Sandbox home');
