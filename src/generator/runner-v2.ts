...
function generateInstallBlock(app: AppManifest): string {
  ...
  if (app.detection.type === 'file') {
    ...
    lines.push(`$detectionResult = & powershell.exe -NoProfile -Command ${psQuote(app.detection.value)}; if ($LASTEXITCODE -ne 0) { throw ${psQuote(`${app.name} command detection failed after provisioning.`)} }`);
  }
...
function Invoke-ToolAction {
    param($Action)
    try {
        $verb = if ($Action.elevated -eq $true) { 'RunAs' } else { $null }
        if ($Action.type -eq 'folder') {
            Start-Process -FilePath 'explorer.exe' -ArgumentList @($Action.path) | Out-Null
            return
        }
        ...
        if ($verb) {
            Start-Process -FilePath $target -ArgumentList $args -Verb $verb | Out-Null
        } else {
...
foreach ($name in $Names) {
    ...
    if ($actions.Count -eq 1) {
        ...
        $handler = { Invoke-ToolAction $selectedAction }.GetNewClosure()
...
function generateActionProvisioning(apps: AppManifest[]): string {
  const blocks: string[] = [];
  for (const app of apps) {
    for (const [index, action] of (app.actions ?? []).entries()) {
      const actionJson = jsonBase64({ ...action, appName: app.name, index });
      blocks.push(`New-ToolAction -EncodedAction '${actionJson}'`);
    }
  }
  return blocks.join('\n');
}
...
export function generateRunner(apps: AppManifest[]): string {
  ...
  const uiScript = generateUi(stepNames, actionMap(apps));
  ...
  const appBlocks = apps
    ...
    .join('\n\n');
  const actionBlocks = generateActionProvisioning(apps);
...
function Invoke-ProvisionStep {
  ...
  catch { $script:Failures += "$($Name): $($_.Exception.Message)"; Write-StepState -Name $Name -Status 'FAILED'; Write-Log "FAILED: $($Name) - $($_.Exception.Message)"; Write-Warning "$($Name) failed: $($_.Exception.Message)" }
}
...
function Test-GoogleDnsDirect { try { return [bool](Resolve-DnsName 'www.microsoft.com' -Server '8.8.8.8' -Type A -QuickTimeout -ErrorAction Stop) } catch { return $false } }
function Invoke-DnsRecovery {
  if (Test-MicrosoftDnsProbe) { return }
  $dnsServer = '8.8.8.8'
  if (-not (Test-GoogleDnsDirect)) { throw 'Direct DNS query to 8.8.8.8 failed.' }
  $configured = $false
  try {
    ...
    Start-Sleep -Seconds 1
    if (Test-MicrosoftDnsProbe) { $configured = $true }
  } catch { Write-Log "PowerShell DNS recovery failed: $($_.Exception.Message)" }
  if (-not $configured) {
  ...
  if (-not $configured) { throw 'DNS recovery failed.' }
}
...
function New-ToolAction {
  param([string]$EncodedAction)
  $json = [Text.Encoding]::UTF8.GetString([Convert]::FromBase64String($EncodedAction))
  $action = ConvertFrom-Json $json
  if ($action.startMenu -eq $false) { return }
  $safe = ($action.name -replace '[\\/:*?"<>|]', '-').Trim()
  $launcher = Join-Path $Launchers ($safe + '.ps1')
  $actionPayload = [Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes((ConvertTo-Json $action -Compress)))
  $launcherCode = @"
`$a = ConvertFrom-Json ([Text.Encoding]::UTF8.GetString([Convert]::FromBase64String('$actionPayload')))
`$verb = if (`$a.elevated -eq `$true) { 'RunAs' } else { `$null }
if (`$a.type -eq 'folder') { Start-Process explorer.exe -ArgumentList @([string]`$a.path); exit }
`$target = if (`$a.type -eq 'app') { [string]`$a.path } else { [string]`$a.command }
`$args = [string]`$a.arguments
if (`$verb) { Start-Process -FilePath `$target -ArgumentList `$args -Verb `$verb } else { Start-Process -FilePath `$target -ArgumentList `$args }
"@
  Set-Content -Path $launcher -Value $launcherCode -Encoding UTF8 -Force
  New-Shortcut -Path (Join-Path $StartMenu ($safe + '.lnk')) -Target 'powershell.exe' -Arguments "-NoLogo -NoProfile -WindowStyle Hidden -ExecutionPolicy Bypass -File `"$launcher`"" -WorkingDirectory $Root
}
