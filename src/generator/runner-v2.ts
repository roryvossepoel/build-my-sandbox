...
function launcherScript(action: AppAction): string {
  ...
  const argumentList = args ? ` -ArgumentList ${psQuote(args)}` : '';
  return `Start-Process -FilePath ${psQuote(target)}${argumentList}${verb}`;
}
  ...
  try {
    ...
    if ($Action.elevated -eq $true) {
      if ([string]::IsNullOrWhiteSpace($args)) {
        Start-Process -FilePath $target -Verb RunAs | Out-Null
      } else {
...
foreach ($name in $Names) {
  ...
  foreach ($action in $actions) {
    ...
    if (-not [string]::IsNullOrWhiteSpace([string]$action.uiLabel)) {
      $actionButton.Content = [string]$action.uiLabel
    } elseif ($actions.Count -eq 1) {
      $actionButton.Content = if ($action.type -eq 'folder') { 'Open folder' } else { 'Start' }
    } else {
      $actionButton.Content = [string]$action.name
    }
    $actionButton.Margin = '0,0,6,0'
    ...
    $actionPanel.Children.Add($actionButton) | Out-Null
  }
